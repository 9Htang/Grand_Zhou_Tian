...
## 预选目标（拖拽到敌人时预设，request() 检测到直接完成）
var preselected_target: Dictionary = {}
...
func request(selector: Dictionary, battle: BattleContext) -> void:
	...
	# 预选目标（拖拽到敌人）→ 直接完成
	if not preselected_target.is_empty():
		selected.append(preselected_target)
		preselected_target = {}
		selection_completed.emit(pending_selector, selected.duplicate())
		return

	pending_targets = registry.get_targets(selector, battle)
	print("[TargetManager.request] candidates=%d count=%d" % [pending_targets.size(), count])

	# 候选数 ≤ 需求数 → 自动完成，不弹 UI（单敌人自动选）
	if pending_targets.size() <= count:
		print("[TargetManager.request] auto-complete — %d candidates" % pending_targets.size())
		selected = pending_targets.duplicate()
		selection_completed.emit(pending_selector, selected.duplicate())
		return
...
## 检查目标是否在合法目标列表中
func is_valid_target(submission: Dictionary) -> bool:
	for t in pending_targets:
		if _target_eq(t, submission):
			return true
	return false


## 预选目标（拖拽到敌人时由 DragInputHandler 调用）
func set_preselected(target: Dictionary) -> void:
