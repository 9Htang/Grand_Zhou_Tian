		...
		"has_artifact":
			for a in gm.artifacts:
				if a.id == parts[1]:
					return true
			return false
		"first_turn": return context.get("elapsed_seconds", 0.0) <= 5.0
	return true
...
static func on_turn_start(gm: Node, elapsed_seconds: float) -> void:
	trigger(gm.artifacts, gm, Trigger.ON_TURN_START, {"elapsed_seconds": elapsed_seconds})
...
## 灵气不够则跳过该法宝，继续判断下一个
static func poll_passive(gm: Node, elapsed_seconds: float) -> void:
	...
	for artifact: ArtifactData in gm.artifacts:
		if artifact.artifact_type == "passive" or artifact.artifact_type == "active_mount":
			...
			if artifact.qi_per_turn > 0:
				if not ectx.qi.spend_qi( artifact.qi_per_turn):
					continue  # 灵气不够，跳过此法宝
			# 检查触发条件
			if not artifact.condition.is_empty():
				if not _check_condition(gm, artifact.condition, {"elapsed_seconds": elapsed_seconds}):
					continue
			# 执行法宝效果
			if not artifact.effect.is_empty():
				EffectResolver.apply(gm, artifact.effect)
