...
static func tick(gm: Node, flow_tracker: Dictionary = {}) -> Dictionary:
	...
	# 1. 丹田注入灵气到相邻穴位
	var injected: bool = _inject_from_dantian(gm, meridian)
	...
	if not unlocked_nodes.is_empty():
		result["nodes_unlocked"] = unlocked_nodes
...
static func _inject_from_dantian(gm: Node, meridian: MeridianMapData) -> bool:
	if gm.dantian_qi <= 0:
		return false
